...
public class FeeHandlerImpl implements FeeHandler {
    ...
    private final DictDB<Address, BigInteger> swapFeesAccruedDB = Context.newDictDB(DIVIDEND_TOKENS, BigInteger.class);
    private BigInteger loanFeesAccrued;
    private BigInteger stabilityFundFeesAccrued;
    ...
    public void setSwapFeesAccruedDB() {
        for (int i = 0; i < acceptedDividendsTokens.size(); i++) {
            ...
            swapFeesAccruedDB.set(thisToken, swapFeesAccruedDB.get(thisToken));
        }
    }

    @External(readonly = true)
    public BigInteger getSwapFeesAccruedByToken(Address token) {
        return swapFeesAccruedDB.get(token);
    }
    ...
    public void tokenFallback(Address _from, BigInteger _value, byte[] _data) {
        ...
        if (_from == loans.get()) {
            loanFeesAccrued = loanFeesAccrued.add(_value);
        }

        if (_from == dex.get() && swapFeesAccruedDB.get(sender) != null) {
        ...
        if (_from == stabilityFund.get()) {
            stabilityFundFeesAccrued = stabilityFundFeesAccrued.add(_value);
        }
    }
    ...
    public BigInteger getLoanFeesAccrued() {
        return loanFeesAccrued;
    }

    @External(readonly = true)
    public BigInteger getStabilityFundFeesAccrued() {
        return stabilityFundFeesAccrued;
    }
    // add swap fees getter
