...
import static network.balanced.score.core.DBHelpers.*;
...
public class Position {
    ...
    public Standing getStanding(int day, Boolean readOnly) {
        ...
        if (Loans._getDay() < Loans.continuousRewardDay.get()) {
            if (standing.ratio.compareTo(Loans.miningRatio.get().multiply(EXA).divide(POINTS)) == 1) {
                ...
                if (bnusdDebt.compareTo(Loans.minMiningDebt.get()) == -1) {
                    standing.standing = Standings.NOT_MINING;
                } else {
                    standing.standing = Standings.MINING;
                }
            } else if (standing.ratio.compareTo(Loans.lockingRatio.get().multiply(EXA).divide(POINTS)) == 1) {
                standing.standing = Standings.NOT_MINING;
            } else if (standing.ratio.compareTo(Loans.liquidationRatio.get().multiply(EXA).divide(POINTS)) == 1) {
                standing.standing = Standings.LOCKED;                
            } else {
                standing.standing = Standings.LIQUIDATE;
            }
        } else {
            if (standing.ratio.compareTo(Loans.liquidationRatio.get().multiply(EXA).divide(POINTS)) == 1) {
                    standing.standing = Standings.MINING;
            } else {
                    standing.standing = Standings.LIQUIDATE;
            }
    ...
    public Constants.Standings updateStanding(int day) {
    ...
    public Map<String, Object> toMap(int day) {
        ...
        return Map.ofEntries(
            ...
            entry("standing", Constants.StandingsMap.get(standing.standing))
        );
    }
